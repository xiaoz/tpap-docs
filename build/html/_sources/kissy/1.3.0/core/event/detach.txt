...
.. function:: detach
    ...
    .. code-block:: javascript
        ...
        KISSY.Event.detach('#foo');

    上面的代码解除了 foo 元素上所有事件的事件处理器, 我们也可以解除某一个事件的全部事件处理器:

    .. code-block:: javascript

        KISSY.Event.detach('#foo','click');
    ...
    .. code-block:: javascript
        ...
        KISSY.Event.on('#foo','click', handler);
        KISSY.Event.detach('#foo','click', handler);
    ...
    .. code-block:: javascript
        ...
        var obj={x:1};
        KISSY.Event.on('#foo','click', handler,obj);

        KISSY.Event.detach('#foo','click', function() {
          console.log('The quick brown fox jumps over the lazy dog.');
        },obj);

        KISSY.Event.detach('#foo','click', handler,{x:1});
